...
**First, read the environment receipt if the PR has one.** PRs produced by `/implement-plan` carry a `## 环境收据` block in the body (base SHA, venv isolation, config.yaml/live handling, `N passed/M skipped/K failed`). Grep it out first — it lets you skip *exploratory* reruns. **The suite and the tests are already done — don't redo them.** `/implement-plan` had the implementation session run the suite + its various tests once before pushing (its step 3), and this receipt is that run's record. Your rerun is the fact-checks below + the changed-area neighborhood + the per-failure recheck (§4), **not** a re-run of the author's suite.
